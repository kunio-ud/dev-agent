...
| **基本設計 → 単体テスト設計／コーディングルール** | **(1) `2.4 アーキテクチャ方針・実装制約` の方針合意が完成（採用ライブラリのバージョン、レイヤー責務、横断ライブラリの「選定」が具体的に記載／【TODO】が無い）／ (2) TODO 集約レポートに `BLOCKER` が 0 件／ (3) `openapi.yaml` が SSoT として存在し `x-api-id`・`operationId` が全 operation に付与済み／ (4) 主要シーケンス図に準正常系を含む／ (5) 採用ライブラリとテストランナー・ランタイムバージョンの互換性を実際に `npm test`（または相当コマンド）で確認済み（native addon ビルド可否、ESM/CJS 混在、builtinModules の扱いなどを含む）** | `basic-design-doc` に戻して該当章を補強。BLOCKERは顧客／PM確認 |
