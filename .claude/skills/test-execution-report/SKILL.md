...
本スキルは「エビデンス」を収集し、`traceability.json` の結果を更新し、`evidence.json` を出力してリリース判定へ繋ぐ。
...
- `traceability.json`（テストケース ID 一覧）
...
1. **結果の記録**
   ...
   - `traceability.json` の `tests` 配下の `result` を更新する。
...
4. **証拠パッケージ（evidence.json）の作成**
   - `metrics` に `test_count`, `test_success`, `test_failure` を記入する。
   - 未解決の `blockers`（クリティカルな障害）件数を明記する。
   - `artifacts` に証跡ファイル群のパスを含める。

## 完了報告

- テスト実施報告書（サマリ）
- 障害一覧（Defect List）
- 更新された `traceability.json`
- `evidence.json`（リリース判定の主入力）
- 残リスク（未実施・未解決事項の影響範囲）
