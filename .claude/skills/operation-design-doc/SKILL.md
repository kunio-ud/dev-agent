...
本スキルは単なるドキュメント作成だけでなく、**「運用開始が可能であること」を証明（evidence.json）**し、リリース判定へ繋ぐ。
...
- `traceability.json`（非機能要件 NFR-ID の達成状況）
- `evidence.json`（テスト結果：性能テストやセキュリティテストの Pass 状態）
...
4. **運用エビデンス（evidence.json）の作成**
   - `phase` を `operation-design` に設定。
   - `artifacts` に運用設計書、Runbook のパスを含める。
